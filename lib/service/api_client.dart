...
// এই ফাংশনটি রিপ্লেস করুন
  static Future<Response> postMultipartData(String uri, dynamic body, {List<MultipartBody>? multipartBody, Map<String, String>? headers}) async {
    try {
      ...
      var mainHeaders = {
        'Accept': 'application/json',
        'Authorization': bearerToken, // টোকেন লাগলে আনকমেন্ট করুন
      };

      debugPrint('====> API Call: $uri\nHeader: ${headers ?? mainHeaders}');
      debugPrint('====> API Body: $body with ${multipartBody?.length} picture');

      var request = http.MultipartRequest('POST', Uri.parse(ApiUrl.baseUrl + uri));
      ...
      // ================== FIX: JSON Decode Added Here ==================
      dynamic responseBody;
      try {
        responseBody = jsonDecode(content);
      } catch (e) {
  ...
  // static Future<Response> getWithRefresh(String uri, {Map<String, dynamic>? query, Map<String, String>? headers}) async {
  //   Response response = await getData(uri, query: query, headers: headers);
  //
  //   if (response.statusCode == 401) {
  //     bool refreshed = await Get.find<AuthController>().refreshAccessToken();
  //     if (refreshed) {
  //       response = await getData(uri, query: query, headers: headers);
  //     }
  //   }
  //
  //   return response;
  ...
  // static Future<Response> postWithRefresh(String uri, dynamic body, {Map<String, String>? headers, bool isContentType = true}) async {
  //   Response response =
  //   await postData(uri, body, headers: headers, isContentType: isContentType);
  //
  //   if (response.statusCode == 401) {
  //     bool refreshed = await Get.find<AuthController>().refreshAccessToken();
  //     if (refreshed) {
  ...
  // static Future<Response> patchWithRefresh(String uri, dynamic body, {Map<String, String>? headers, bool isContentType = true}) async {
  //   Response response =
  //   await patchData(uri, body, headers: headers, isContentType: isContentType);
  //
  //   if (response.statusCode == 401) {
  //     bool refreshed = await Get.find<AuthController>().refreshAccessToken();
  //     if (refreshed) {
  ...
  // static Future<Response> putWithRefresh(String uri, dynamic body, {Map<String, String>? headers}) async {
  //   Response response = await putData(uri, body, headers: headers);
  //
  //   if (response.statusCode == 401) {
  //     bool refreshed = await Get.find<AuthController>().refreshAccessToken();
  //     if (refreshed) {
  //       response = await putData(uri, body, headers: headers);
  //     }
  //   }
  //
  //   return response;
